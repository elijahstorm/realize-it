    ...
    React.useEffect(() => {
        ...
        const check = async () => {
            try {
                ...
                const appRoles = (user.app_metadata?.roles || user.user_metadata?.roles) as
                    | string[]
                    | undefined
                const roleFlags = ['is_admin', 'admin', 'is_merchant', 'merchant', 'seller']
                let isAdmin = false
                if (Array.isArray(appRoles)) {
                    isAdmin =
                        appRoles.includes('admin') ||
                        appRoles.includes('merchant') ||
                        appRoles.includes('seller')
                }
                if (!isAdmin) {
                    for (const k of roleFlags) {
                        if (
                            (user.user_metadata as any)?.[k] === true ||
                            (user.app_metadata as any)?.[k] === true
                        ) {
                            isAdmin = true
                            break
                        }
    ...
    return (
        <SidebarProvider defaultOpen>
            <div className="flex min-h-svh">
                ...
                <SidebarInset className="bg-background flex min-h-svh flex-1 flex-col">
